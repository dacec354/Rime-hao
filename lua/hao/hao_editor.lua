...
function this.func(key_event, env)
  ...
  if not (rime_api.regex_match(incoming, "[weruio]") or incoming == "BackSpace") then
    return snow.kNoop
  end
  -- 判断是否满足补码条件：末音节有 4 码，且前面至少还有一个音节
  ...
  if not rime_api.regex_match(current_input, ".+[zyxvtsqpnmlkjhgfdcba][weruio]{4}") then
    return snow.kNoop
  end
  -- 如果输入是 Backspace，还要验证是否有补码
  if incoming == "BackSpace" then
    if not rime_api.regex_match(current_input, "[zyxvtsqpnmlkjhgfdcba][weruio]+.+") then
      return snow.kNoop
    end
  end
  -- 找出补码的位置（第二个音节之前），并添加补码
  local first_char_code_len = current_input:find("[zyxvtsqpnmlkjhgfdcba]", 2) - 1
