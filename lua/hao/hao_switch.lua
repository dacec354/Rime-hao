...
更新[by 荒]:
- 2025-07-16: 切换方案时，清空 namespaces 缓存
---------------------------------------------------------------------------
]]

local hao_switch_proc = {} -- 開關管理-processor
local hao_switch_tr   = {} -- 開關管理-translator
...
local namespaces = {}  -- 必须先定义
local last_schema_id = nil
local function clear_namespaces()
    for k in pairs(namespaces) do
        if k ~= "init" and k ~= "set_config" and k ~= "config" then
            namespaces[k] = nil
        end
    end
end
...
function namespaces:init(env)
    ...
    if last_schema_id ~= current_schema_id then
        clear_namespaces()
        last_schema_id = current_schema_id
    end
    -- 讀取配置項
...
function hao_switch_proc.init(env)
    if Switcher then
...
function hao_switch_proc.func(key_event, env)
...
function hao_switch_proc.fini(env)
end
...
function hao_switch_tr.init(env)
    -- 讀取配置項
    local ok = pcall(namespaces.init, namespaces, env)
    if not ok then
        local config = {}
...
function hao_switch_tr.func(input, seg, env)
    local ctx = env.engine.context
    local funckeys = namespaces:config(env).funckeys
    if funckeys.macro[string.byte(string.sub(ctx.input, 1, 1))] then
...
function hao_switch_tr.fini(env)
end

-- ######## RETURN ########

return {
    proc = hao_switch_proc, -- 開關管理-processor
    tr   = hao_switch_tr,   -- 開關管理-translator
}
