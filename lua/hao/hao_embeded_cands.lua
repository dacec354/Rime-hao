-- 将要被返回的過濾器對象
local hao_embeded_cands_filter = {}
local core = require("hao.hao_core")

-- 添加一个简单的 translator 来设置 input_code 和 stashed_text
local hao_embeded_cands_translator = {}

function hao_embeded_cands_translator.init(env)
    -- 初始化时清空
    core.input_code = ''
    core.stashed_text = ''
end

function hao_embeded_cands_translator.func(input, segment, env)
    -- 设置当前输入串
...
function hao_embeded_cands_translator.fini(env)
    -- 清理
    core.input_code = ''
    core.stashed_text = ''
end

-- 将 translator 添加到返回的表中
hao_embeded_cands_filter.translator = hao_embeded_cands_translator
...
function hao_embeded_cands_filter.init(env)
    -- 讀取配置項
    ...
    -- 構造回調函數
    local option_names = {
        [core.switch_names.hao_embeded_cands] = true,
    }
...
-- 過濾器
function hao_embeded_cands_filter.func(input, env)
    if not env.option[core.switch_names.hao_embeded_cands] then
...
function hao_embeded_cands_filter.fini(env)
    env.option = nil
end

return hao_embeded_cands_filter
