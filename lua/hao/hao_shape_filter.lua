...
function filter.handle_candidate(text, shape_input, env)
  ...
  if id == "snow_sipin" then -- 冰雪四拼
    if shape_input:len() > 0 or rime_api.regex_match(current, "[zyxvtsqpnmlkjhgfdcba][weruio]{4}") then
      local code = ""
      local partial_code = ""
      ---@type string?
      local prompt = ""
      ...
      if shape_input:sub(1, 1) == "1" then
        ...
        code = encode(element, { ["一"] = "e", ["丨"] = "i", ["丿"] = "u", ["丶"] = "o", ["乙"] = "r" })
        prompt = partial_code:len() > 0 and
        " 笔画 [" .. partial_code:gsub(".", { ["e"] = "一", ["i"] = "丨", ["u"] = "丿", ["o"] = "丶", ["r"] = "乙" }) .. "]" or
        nil
        comment = code
      end
